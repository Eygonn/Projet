...
// Envoi automatiquement un ennemi vers notre personnag
void action_ennemi(SDL_Rect *ennemi_destination, SDL_Rect *pers_destination, SDL_Rect *ennemi_source, int frame, int DIM_SPRITE, SDL_Renderer *rendu) {
    ...
    int direction = 0;
    int pos_x = abs(ennemi_destination->x - pers_destination->x);
    int pos_y = abs(ennemi_destination->y - pers_destination->y);

    if( (ennemi_destination->x != pers_destination->x) && (ennemi_destination->y && pers_destination->y) ){

        if(ennemi_destination->x)
        // Calcul de la direction vers la cible
        if (ennemi_destination->x < pers_destination->x - 50) {
            ennemi_destination->x += VITESSE_JOUEUR / 1.5;
            direction = 2; // Droite
        } else if (ennemi_destination->x > pers_destination->x + 50) {
            ennemi_destination->x -= VITESSE_JOUEUR / 1.5;
            direction = 3; // Gauche
        }
        if (ennemi_destination->y < pers_destination->y - 50) {
            ennemi_destination->y += VITESSE_JOUEUR / 1.5;
            direction = 0; // Bas
        } else if (ennemi_destination->y > pers_destination->y + 50) {
            ennemi_destination->y -= VITESSE_JOUEUR / 1.5;
            direction = 1; // Haut
        }

        actualisationSprite(6, frame, DIM_SPRITE, DIM_SPRITE, direction, ennemi_source, ennemi_destination, rendu);
    }
    actualisationSprite(6, frame, DIM_SPRITE, DIM_SPRITE, direction, ennemi_source, ennemi_destination, rendu);


    // Il faudrait que l'ennemi passe à l'attaque 
}
